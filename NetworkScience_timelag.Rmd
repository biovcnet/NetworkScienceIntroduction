...
Jake Example
```{r}
df <- data.frame(Var1=c("A","A","A","B","B","C"),
                 Var2=c("B","C","D","C","D","D"),
                 Lag= c(1,0,-1,0,1,-1),
                 stringsAsFactors = F)

spp <- unique(c(df$Var1,df$Var2))

x <- matrix(0,nrow=length(spp),ncol=length(spp))
rownames(x) <- spp
colnames(x) <- spp
for(i in 1:nrow(df)){
  if(df[i,"Lag"]==1){
    x[df[i,"Var1"],df[i,"Var2"]] <- 1
  } else if(df[i,"Lag"]==-1){
    x[df[i,"Var2"],df[i,"Var1"]] <- 1
  }

}

grph <- graph_from_adjacency_matrix(x)
plot(grph)
```
