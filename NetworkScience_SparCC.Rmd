...
It is is recommended to remove any rows and columns with fewer than two reads on average.

```{r}
SpeciesToKeep <- apply(TaraPhylaMtx, 2, mean) > 2
SitesToKeep <- apply(TaraPhylaMtx, 1, mean) > 2
TaraPhylaMtx <- TaraPhylaMtx0[SitesToKeep,SpeciesToKeep]
```
...
spearCor_plot <- spearCor_toPlot %>% ggplot(aes(x = Var2, y = Var1, fill = value)) + geom_tile() + scale_fill_gradient2() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
spearCor_plot
```
...
```{r}
TaraPhylaMtxClr <- compositions::clr(TaraPhylaMtx)
```
