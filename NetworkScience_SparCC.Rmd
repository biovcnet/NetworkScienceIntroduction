...
spearCorTestClr <- corr.test(TaraPhylaMtx, method = "spearman", adjust = "none")
...
spearRhoP_Clr <- left_join(spearCor_processed, spearP_processed, by = c("Var1", "Var2")) %>%
  # # remove self correlations
  # filter(Var1 != Var2) %>% 
  # calculate the false discovery rate to adjust for multiple p values
  mutate(fdr = p.adjust(p, method = "BH"))
...
```{r}
tp0 <- proc.time()
out2 <- sparccboot(TaraPhylaMtx, R = 100, ncpus = 7)
tp1 <- proc.time()
tp1 - tp0
```
...
## Plotting correlation matrix with fdr rates
...
```{r}
write_csv(AllTaraNetworkStats, "TaraOceansAllAnalysies_p100.csv")
```
