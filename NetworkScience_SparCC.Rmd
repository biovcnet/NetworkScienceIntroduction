...
spearCor_toPlot <- spearCor %>% reorder_cormat() %>% get_upper_tri() %>% reshape2::melt() %>% na.omit()
```
...
spearCor_toPlot %>% ggplot(aes(x = Var2, y = Var1, fill = value)) + geom_tile() + scale_fill_gradient2() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
```{r}
spearCorClr <- cor(TaraPhylaMtxClr, method = "spearman")
```
...
spearCorClr_toPlot %>% ggplot(aes(x = Var2, y = Var1, fill = value)) + geom_tile() + scale_fill_gradient2() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
plotableSparcc %>% ggplot(aes(x = Var2, y = Var1, fill = value)) + geom_tile() + scale_fill_gradient2() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
